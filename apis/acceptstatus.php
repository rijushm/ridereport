	...
	if (mysqli_num_rows($res)) {
		while ($row = mysqli_fetch_array($res)) {
			if ($row['status'] == 'pending') {
				...
				$upquery = "update `requests` set `status` = 'approved', `updateTime` = '$uptime' where `uid` = '$uid' and `employee` = '$email'";
				$upres = mysqli_query($conn, $upquery);
				if ($upres) {
					if (deviceId($conn, $email) != false) {
						$deviceId = deviceId($conn, $email);
						include '../assets/views/sendacceptnotice.php';
					}
					include '../assets/views/status/accept.php';
				}else{
